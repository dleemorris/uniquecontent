<?php
/**
 * User Content Rest Aux
 *
 * @since 1.0.0
 *
 * @package    Unique_User_Content
 * @subpackage Unique_User_Content/admin/partials/rest/v1
 */
...
class Unique_User_Content_REST_aux {
	...
	public function get_schema( $request ) {
		...
		$this->schema = array(
			...
			'title'      => esc_html__( 'REST Output', 'unique-user-content' ),
			'type'       => 'object',
			// In JSON Schema you can specify object properties in the properties attribute.
			'properties' => array(

				'result' => array(
					'description' => esc_html__( 'Result', 'unique-user-content' ),
					'type'        => 'boolean',
					'context'     => array( 'view' ),
					'readonly'    => true,
					'default'     => false,
				...
				'data'   => array(
					'description' => esc_html__( 'Data', 'unique-user-content' ),
					'type'        => 'array',
					'context'     => array( 'view' ),
					'readonly'    => true,
					'default'     => '',
	...
	public function validate_number( $value, $request, $param ) {
		...
		if ( isset( $attributes['args'][ $param ] ) ) {
			...
			if ( 'number' === $argument['type'] && ! is_numeric( $value ) ) {
				return new WP_Error( 'rest_invalid_param', sprintf( esc_html__( '%1$s is not of type %2$s', 'unique-user-content' ), $param, 'string' ), array( 'status' => 400 ) );
			}
		}
	...
	public function sanitize_number( $value, $request, $param ) {
		...
		return new WP_Error( 'rest_api_sad', esc_html__( 'Something went terribly wrong.', 'unique-user-content' ), array( 'status' => 500 ) );
